...
namespace fCraft {
    ...
    public sealed class CommandDescriptor : IClassy {
        ...
        /// <summary> Custom colors for spells in the descriptor, color-code using the 3 standard
        /// main colors: Strength &c, Focus &e, and Elemental &9. Use these colors in combination 
        /// for combo attacks. Use &7, &8, &f, and &0 for all other commands </summary>
        public string SpellNameColor { get; set; }

        /// <summary> The first number in the array should be the JLWGL KeyCode (http://minecraft.gamepedia.com/Key_Codes) and the 
        /// second should be the KeyMod (0 - None, 1 - Ctrl, 2 - Shift, 4 - Alt) </summary>
        ...
        public string ClassyName 
        {
            get 
            {
                ...
                {
                    if (Category == CommandCategory.Spell)
                    {
                        if (!String.IsNullOrEmpty(Color.Parse(SpellNameColor)))
                            return SpellNameColor + Name;
                    }
                    Rank minRank;
