                    ...
                    if (world.gunPhysics)
                    {
                        ...
                        if (e.Block == Block.TNT && world.tntPhysics)
                        {
                            if (e.Player.CanFireTNT())
                            {
                                ...
                                world.AddPhysicsTask(new Particle(world, e.Coords, dir, e.Player, Block.TNT, _tntBulletBehavior, null), 0);
                            }
                        }
                        else
                        {
                            ...
                            double phi = 2.0 * Math.PI * (e.Player.Position.R - 64) / 256.0;
                            Vector3F dir = new Vector3F((float)(r * Math.Cos(phi)), (float)(r * Math.Sin(phi)), (float)(Math.Sin(ksi)));
                            world.AddPhysicsTask(new Particle(world, e.Coords, dir, e.Player, block, _bulletBehavior, null), 0);
                        }
                    }
                }
            }
