...
namespace fCraft
{
    public class TNTTask : PhysicsTask
    {
        ...
        public TNTTask(World world, Vector3I position, Player owner, bool gun, bool particles, Command cmd)
            : base(world)
        {
        ...
        private void CreateParticles()
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                if (_owner.CanPlace(_map, (_pos + 2 * direction).Round(), Block.Obsidian, BlockChangeContext.Manual) == CanPlaceResult.Allowed)
                {
                    ...
                    {
                        _world.AddPhysicsTask(new Particle(_world, (_pos + 2 * direction).Round(), direction, _owner, Block.Obsidian, _particleBehavior, _cmd), 0);
                    }
                }
            }
        }
        ...
        private void TryAddPoint(int x, int y, int z)
        {
            ...
            if (Block.Lava != prevBlock)
            {
                ...
                {
                    _world.AddPhysicsTask(new TNTTask(_world, new Vector3I(x, y, z), _owner, false, _particles, null), _r.Next(150, 300));
    ...
    public class SelfDestructTask : PhysicsTask
    {
        private struct BData
        {
            public int X, Y, Z;
        ...
        public SelfDestructTask(World world, Vector3I position, Player owner, Command cmd, bool gun, bool particles)
            : base(world)
        {
            _particles = particles;
            _gun = gun;
            ...
            lock (_world.SyncRoot)
            {
                switch (_stage)
                {
                   ...
                   /*     if (!_gun)
                        ...
                        //UpdateMap(new BlockUpdate(null, _pos, Block.Air)); no block is placed to remove
        ...
        private void CreateParticles()
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                if (_map.GetBlock((_pos + 2 * direction).Round()) != Block.Obsidian)
                {
                    _world.AddPhysicsTask(new Particle(_world, (_pos + 2 * direction).Round(), direction, _owner, Block.Obsidian, _particleBehavior, null), 0);
                }
            }
        } 
        ...
        private bool Explosion()
        {
            ...
            if (++_currentR <= R)
            {
                ...
                foreach (BData pt in _explosion)
                {
                    if (_map.GetBlock(pt.X, pt.Y, pt.Z) != Block.Lava) //if another tnt hasnt already caused lava here
                    {
                        ...
                        foreach (Player p in _world.Players)
                        {
                            if (p.CanBeKilled() && p.Position.DistanceSquaredTo((new Vector3I(pt.X, pt.Y, pt.Z)).ToPlayerCoords()) <= 96 * 96) //less than or equal to 3 blocks
                                HitPlayer(_world, p, _owner, _cmd);
                        }
                    }
                }
            }
            ...
            if (null != toClean)
            {
                ...
                {
                    UpdateMap(new BlockUpdate(null, (short)pt.X, (short)pt.Y, (short)pt.Z,
                        pt.PrevBlock == Block.Water ? Block.Water : pt.PrevBlock));
                }
                toClean.Clear();
            }
            return _currentR <= R;
        ...
        public void HitPlayer(World world, Player hitted, Player by, Command cmd)
        {
            hitted.Damage(SpellUtils.SelfDestructDamage(by, hitted), by, cmd);
        }
    }
}
