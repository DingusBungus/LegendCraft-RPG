...
namespace fCraft
{
    public interface IParticleBehavior
    {
        ...
        void HitPlayer(World world, Vector3I pos, Player hitted, Player by, ref int restDistance, IList<BlockUpdate> updates, Command cmd);
    }
    ...
    public class Particle : PhysicsTask
    {
        ...
        /// <param name="behavior">The partcle behavior. Includes how far and fast it moves, what happens if it hits a player an obstacle etc.</param>
        public Particle(World world, Vector3I pos, Vector3F direction, Player owner,
            Block block, IParticleBehavior behavior, Command cmd)
            : base(world)
        {
        ...
        private void CheckHitPlayers(List<BlockUpdate> updates)
        {
            foreach (Player p in _world.Players) 
            {
                if (ReferenceEquals(p, _owner) && (_startingPos - _pos).LengthSquared <= 2 * 2) //do not react on owner within 2 blocks of the starting position
                    continue;
                if (p.Position.DistanceSquaredTo(_pos.ToPlayerCoords()) <= 64 * 64) //less than or equal to 2 blocks
                    _behavior.HitPlayer(_world, _pos, p, _owner, ref _restDistance, updates, _cmd);
            }
        }
    }
    ...
    internal class ExplosionParticleBehavior : IParticleBehavior
    {
        ...
        public bool VisitBlock(World world, Vector3I pos, Block block, Player owner, ref int restDistance, IList<BlockUpdate> updates, Block sending)
        {
            ...
            {
                world.AddPhysicsTask(new TNTTask(world, pos, owner, false, true, null), _r.Next(150, 300));
            }
        ...
        public void HitPlayer(World world, Vector3I pos, Player hitted, Player by, ref int restDistance, IList<BlockUpdate> updates, Command cmd)
        {
    ...
    internal class BulletBehavior : IParticleBehavior
    {
        ...
        public bool VisitBlock(World world, Vector3I pos, Block block, Player owner, ref int restDistance, IList<BlockUpdate> updates, Block sending)
        {
            if (world.Map.GetBlock(pos) != Block.Air && Block.Air != block && Block.Water != block && Block.Lava != block)
            {
                restDistance = 0;
                return true;
            }
        ...
        public void HitPlayer(World world, Vector3I pos, Player hitted, Player by, ref int restDistance, IList<BlockUpdate> updates, Command cmd)
        {            
            //NOTE: Does not currently work, HitPlayer does not get called in most instances of the use
            Server.Message("Calling Hit Player");
            double damage;
            if (cmd == null)
            {
                hitted.Kill(world, hitted.ClassyName + " &swas shot by " + by.ClassyName);
            }
            else
            {
                if (cmd.Name.ToLower() == "fissure")
                    damage = SpellUtils.FissureDamage(by, hitted);
                else
                    damage = SpellUtils.ElementalSpearDamage(by, hitted);
                hitted.Damage(damage, by, cmd);
                hitted.Message("{0}&s has dealt {1:0.00}&s damage with {2}", by.ClassyName, damage, cmd.Descriptor.ClassyName);
            }
            updates.Add(new BlockUpdate(null, pos, Block.Air));
            restDistance = 0;
        }
    ...
    internal class TntBulletBehavior : IParticleBehavior
    {
        ...
        public bool VisitBlock(World world, Vector3I pos, Block block, Player owner, ref int restDistance, IList<BlockUpdate> updates, Block sending)
        {
            if (Block.Air != block && Block.Water != block) //explode it
            {
                ...
                world.AddPhysicsTask(new TNTTask(world, pos, owner, false, true, null), 0);
                restDistance = 0;
                return false;
            }
            return true;
        ...
        public void HitPlayer(World world, Vector3I pos, Player hitted, Player by, ref int restDistance, IList<BlockUpdate> updates, Command cmd)
        {
            ...
            updates.Add(new BlockUpdate(null, pos, Block.TNT));
            world.AddPhysicsTask(new TNTTask(world, pos, by, false, true, null), 0);
            restDistance = 0;
        }
    }
