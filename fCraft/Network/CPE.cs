...
namespace fCraft
{
    public sealed partial class Player
    {
        ...
        const string ClickDistanceName = "ClickDistance";
        const int ClickDistanceExtVersion = 1;

        const string HeldBlockName = "HeldBlock";
        const int HeldBlockExtVersion = 1;

        const string TextHotKeyName = "TextHotKey";
        const int TextHotKeyExtVersion = 1;

        const string ExtPlayerListName = "ExtPlayerList";
        const int ExtPlayerListExtVersion = 1;

        const string EnvColorsName = "EnvColors";
        const int EnvColorsExtVersion = 1;

        const string ChangeModelName = "ChangeModel";
        const int ChangeModelExtVersion = 1;

        const string EnvMapAppearanceName = "EnvMapAppearance";
        const int EnvMapAppearanceExtVersion = 1;

        const string EnvWeatherTypeName = "EnvWeatherType";
        const int EnvWeatherTypeExtVersion = 1;

        const string HackControlName = "HackControl";
        const int HackControlExtVersion = 1;

        const string MessageTypesName = "MessageTypes";
        ...
        bool NegotiateProtocolExtension()
        {
            ...
            writer.Write(Packet.MakeExtEntry(SelectionBoxExtName, SelectionBoxExtVersion).Data);
            writer.Write(Packet.MakeExtEntry(ClickDistanceName, ClickDistanceExtVersion).Data);
            writer.Write(Packet.MakeExtEntry(HeldBlockName, HeldBlockExtVersion).Data);
            writer.Write(Packet.MakeExtEntry(TextHotKeyName, TextHotKeyExtVersion).Data);
            writer.Write(Packet.MakeExtEntry(ExtPlayerListName, ExtPlayerListExtVersion).Data);
            writer.Write(Packet.MakeExtEntry(EnvColorsName, EnvColorsExtVersion).Data);
            writer.Write(Packet.MakeExtEntry(ChangeModelName, ChangeModelExtVersion).Data);
            writer.Write(Packet.MakeExtEntry(EnvWeatherTypeName, EnvWeatherTypeExtVersion).Data);
            writer.Write(Packet.MakeExtEntry(HackControlName, HackControlExtVersion).Data);
            writer.Write(Packet.MakeExtEntry(EnvMapAppearanceName, EnvMapAppearanceExtVersion).Data);
            writer.Write(Packet.MakeExtEntry(MessageTypesName, MessageTypesExtVersion).Data);
            ...
            for (int i = 0; i < expectedEntries; i++)
            {
                ...
                else if (extName == ChangeModelName && extVersion == ChangeModelExtVersion)
                {
                    SupportsChangeModel = true;
                    clientExts.Add(extName + " " + extVersion);
                }
                else if (extName ==  ClickDistanceName && extVersion == ClickDistanceExtVersion)
                {
                    SupportsClickDistance = true;
                    clientExts.Add(extName + " " + extVersion);
                }
                else if (extName == EnvColorsName && extVersion == EnvColorsExtVersion)
                {
                    SupportsEnvColors = true;
                    clientExts.Add(extName + " " + extVersion);
                }
                else if (extName == EnvMapAppearanceName && extVersion == EnvMapAppearanceExtVersion)
                {
                    SupportsEnvMapAppearance = true;
                    clientExts.Add(extName + " " + extVersion);
                }
                else if (extName == EnvWeatherTypeName && extVersion == EnvWeatherTypeExtVersion)
                {
                    SupportsEnvWeatherType = true;
                    clientExts.Add(extName + " " + extVersion);
                }
                else if (extName == ExtPlayerListName && extVersion == ExtPlayerListExtVersion)
                {
                    SupportsExtPlayerList = true;
                    clientExts.Add(extName + " " + extVersion);
                }
                else if (extName == HackControlName && extVersion == HackControlExtVersion)
                {
                    SupportsHackControl = true;
                    clientExts.Add(extName + " " + extVersion);
                }
                else if (extName == HeldBlockName && extVersion == HeldBlockExtVersion)
                {
                    SupportsHeldBlock = true;
                    clientExts.Add(extName + " " + extVersion);
                }
                else if (extName == MessageTypesName && extVersion == MessageTypesExtVersion)
                {
                    SupportsMessageTypes = true;
                    clientExts.Add(extName + " " + extVersion);
                }
                else if (extName == TextHotKeyName && extVersion == TextHotKeyExtVersion)
                {
