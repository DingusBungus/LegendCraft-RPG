...
namespace fCraft
{
    ...
    public sealed partial class Player
    {
        ...
        void IoLoop()
        {
            try
            {
                ...
                while (canSend)
                {
                    ...
                    if (Mana < MaxMana())
                    {
                        if ((DateTime.UtcNow - lastManaRegen).TotalMilliseconds > 500)
                        {
                            lastManaRegen = DateTime.UtcNow;
                            GiveMana(2);
                        }
                    }
                    if (HP < MaxHP() && (!Poisoned || isRegenerating))
                    {
                        //Heal, if regenerating, heal more
                        HealthCheck();
                    }
                    //checks if player buffs have worn off, if so set bools
                    BuffCheck();
                    //check if confused
                    ConfuseCheck();
                    //check if poisoned
                    ...
                    while (canSend && packetsSent < Server.MaxSessionPacketsPerTick)
                    {
                        ...
                        if (this.World.spellHappening && packet.OpCode == OpCode.SetBlockServer)
                        {
                            Thread.Sleep(10);
                            this.World.spellSleepTime += 10;
                        }
                        if (this.World.spellSleepTime > 3000)
                        {
                            this.World.spellHappening = false;
                            this.World.spellSleepTime = 0;
                        }
        ...
        bool LoginSequence()
        {
            ...
            IRaces human = new Human();
            if (Info.race == human|| String.IsNullOrEmpty(Info.race.ToString()))
            {
                Info.race = human;
                Race.GiveRaceAttributes(this, human);
                Message("Choose a race! View all races with /Races and details about the races with /help (race)");
            } 
        ...
        internal bool JoinWorldNow([NotNull] World newWorld, bool doUseWorldSpawn, WorldChangeReason reason)
        {
            ...
            else if (ClassiCube && ConfigKey.WoMEnableEnvExtensions.Enabled() && Heartbeat.ClassiCube())
            {
                if (!newWorld.Hax)
                {
                    ...
                    SendNow(PacketWriter.MakeMessage(textLine2, MessageType.TopRight2));
                }
                else
                {
                ...
                SendNow(PacketWriter.MakeMessage(textLine1, MessageType.TopRight1));
            }
            else
            {
                textLine2 = "Loading world " + newWorld.ClassyName;
        ...
        DateTime lastManaRegen;
        DateTime lastHealthRegen;
            ...
            set
            {
                ...
                switch (actualValue)
                {
                    ...
                    case BandwidthUseMode.High:
                        ...
                        entityHidingThreshold = (150* 32) * (130 * 32);
                        partialUpdates = true;
                        skipUpdates = false;
                        movementUpdateInterval = TimeSpan.FromMilliseconds(50);
                        break;
