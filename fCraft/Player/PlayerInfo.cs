...
namespace fCraft
{
    ...
    public sealed partial class PlayerInfo : IClassy
    {
        ...
        /// <summary> Amount of experience points a player has </summary>
        public int XP;
        /// <summary> Whether a player is considered "good" or "bad" (affects the spell particles) </summary>
        public int Alignment;
        /// <summary> The sum of all attributes </summary>
        public int Level 
        { 
            get
            {
            ...
            set
            {
                Level = Vitality + Strength + Defence + Mana + Elemental + MagicDefence + Focus + Wisdom + Agility + Luck;
            } 
        }
        ...
        /// <summary> Determines how much health the user has (HP) </summary>
        public int Vitality;
        /// <summary> Scales damage on strength-based spells/attacks </summary>
        public int Strength;
        /// <summary> Lowers amount of damage taken from strength-based attacks </summary>
        public int Defence;

        //Will/Mana

        /// <summary> Determines how much mana the user has </summary>
        public int Mana;
        /// <summary> Scales damage on elemental-based spells/attacks </summary>
        public int Elemental;
        /// <summary> Lowers amount of elemental damage </summary>
        public int MagicDefence;

        //Skill/Wisdom

        /// <summary> Scales damage on skill-based spells/attacks </summary>
        public int Focus;
        /// <summary> Determines what spells the user can learn </summary>
        public int Wisdom;
        /// <summary> Lowers damage taken from skill-based attacks </summary>
        public int Agility;
        /// <summary> Raises crit chance on all attacks </summary>
        public int Luck;


        public IRaces race;
        
        ///<summary> Player's amount of skill points used to level up attributes. </summary>
        [CanBeNull]
        public int Points = 0;

        //cooldowns bby
        public DateTime LastUsedInfernalWrath, 
                LastUsedEarthquake, 
                LastUsedElementalSpear, 
                LastUsedFissure, 
                LastUsedStrengthBuff,
                LastUsedElementalBuff,
                LastUsedFocusBuff,
                LastUsedBodySlam,
                LastUsedPunch,
                LastUsedHeal,
        ...
        internal static PlayerInfo LoadFormat2(string[] fields)
        {
            ...
            if (fields[14].Length > 0) Int32.TryParse(fields[14], out info.Points);
            ...
            if (fields.Length > 48)
            {
                if (fields[48].Length > 0) Int32.TryParse(fields[48], out info.Agility);
                if (fields[49].Length > 0) Int32.TryParse(fields[49], out info.Alignment);
                if (fields[50].Length > 0) Int32.TryParse(fields[50], out info.Defence);
                if (fields[51].Length > 0) Int32.TryParse(fields[51], out info.Elemental);
                if (fields[52].Length > 0) Int32.TryParse(fields[52], out info.Focus);
                if (fields[53].Length > 0) Int32.TryParse(fields[53], out info.Luck);
                if (fields[54].Length > 0) Int32.TryParse(fields[54], out info.MagicDefence);
                if (fields[55].Length > 0) Int32.TryParse(fields[55], out info.Mana);
                if (fields[56].Length > 0) Int32.TryParse(fields[56], out info.Strength);
                if (fields[57].Length > 0) Int32.TryParse(fields[57], out info.Vitality);
                if (fields[58].Length > 0) Int32.TryParse(fields[58], out info.Wisdom);
                if (fields[59].Length > 0) Int32.TryParse(fields[59], out info.XP);
            }
            if (fields.Length > 59)
            {
                byte temp;
                if (Byte.TryParse(fields[60], out temp))
                {
                    if (Enum.IsDefined(typeof(IRaces), temp))
                        info.race = Race.ParseRace(temp);
                    else
                        info.race = new Human();
                }
                else
        ...
        internal static PlayerInfo LoadFormat0(string[] fields, bool convertDatesToUtc)
        {
            ...
            if (fields.Length > 59)
            {
                ...
                if (Byte.TryParse(fields[60], out temp))
                {
                    if (Enum.IsDefined(typeof(ERaces), temp))
                        info.race = Race.ParseRace(temp);
                    else
                        info.race = new Human();
                }
        ...
        internal static PlayerInfo LoadBinaryFormat0([NotNull] BinaryReader reader)
        {
            ...
            info.Agility = (Int32)reader.ReadInt32();
            info.Alignment = (Int32)reader.ReadInt32();
            info.Defence = (Int32)reader.ReadInt32();
            info.Elemental = (Int32)reader.ReadInt32();
            info.Focus = (Int32)reader.ReadInt32();
            info.Luck = (Int32)reader.ReadInt32();
            info.MagicDefence = (Int32)reader.ReadInt32();
            info.Mana = (Int32)reader.ReadInt32();
            info.Strength = (Int32)reader.ReadInt32();
            info.Vitality = (Int32)reader.ReadInt32();
            info.Wisdom = (Int32)reader.ReadInt32();
            info.XP = (Int32)reader.ReadInt32();
            info.race = Race.ParseRace(reader.ReadByte());
        ...
        internal void Serialize(StringBuilder sb)
        {
            ...
            if (Points > 0) sb.Digits(Points); // 14
            sb.Append(',');
            ...
            if (Agility > 0) sb.Digits(Agility); //48
            sb.Append(',');

            if (Alignment != 0) sb.Digits(Alignment); //49
            sb.Append(',');

            if (Defence > 0) sb.Digits(Defence); //50
            sb.Append(',');

            if (Elemental > 0) sb.Digits(Elemental); //51
            sb.Append(',');

            if (Focus > 0) sb.Digits(Focus); //52
            sb.Append(',');

            if (Luck > 0) sb.Digits(Luck); //53
            sb.Append(',');

            if (MagicDefence > 0) sb.Digits(MagicDefence); //54
            sb.Append(',');

            if (Mana > 0) sb.Digits(Mana); //55
            sb.Append(',');

            if (Strength > 0)sb.Digits(Strength); //56
            sb.Append(',');

            if (Vitality > 0) sb.Digits(Vitality); //57
            sb.Append(',');

            if (Wisdom > 0) sb.Digits(Wisdom); //58
            sb.Append(',');

            if (XP > 0) sb.Digits(XP); //59
            sb.Append(',');

            if (race != null) sb.Append(race.ID()); //60
        }
        ...
        internal void Serialize([NotNull] BinaryWriter writer)
        {
            ...
            Write7BitEncodedInt(writer, Points); // field 14
            ...
            writer.Write(Agility); //48
            writer.Write(Alignment); //49
            writer.Write(Defence); //50
            writer.Write(Elemental); //51
            writer.Write(Focus); //52
            writer.Write(Luck);  //53
            writer.Write(MagicDefence);  //54
            writer.Write(Mana);  //55
            writer.Write(Strength);  //56
            writer.Write(Vitality);  //57
            writer.Write(Wisdom);  //58
            writer.Write(XP);  //59
            writer.Write(race.ToString());  //60
        }
